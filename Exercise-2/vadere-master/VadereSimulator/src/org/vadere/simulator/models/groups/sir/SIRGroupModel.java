...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
    ...
    private final LinkedHashMap<Integer, SIRGroup> groupsById;
    private final Map<Integer, LinkedList<SIRGroup>> sourceNextGroups;
    private AttributesSIRG attributesSIRG;
    private Topography topography;
    /* ### Task 4-3, 5-2: Allows us to compute how much time has passed since last call */
    // <<<<<<<<<<!!!>>>>>>>>>>
    ...
    private int getFreeGroupId() {
        /* ### Task 4-5, 5-3: Allows us to add a specific number of infected pedestrians at start
        * but also to randomly choose for each pedestrian afterwards if necessary. */
        // <<<<<<<<<<!!!>>>>>>>>>>
        if (this.totalInfected < this.attributesSIRG.getInfectionsAtStart()
                || this.random.nextDouble() < this.attributesSIRG.getInfectionRateWhenAdded())
        // <<<<<<<<<<!!!>>>>>>>>>>
        {
    ...
    public void update(final double simTimeInSec) {
        ...
        double deltaTime = simTimeInSec - LastSimulationTime;
        DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();
        if (c.getElements().size() > 0) {
            /* ### Task 4-4: Increase neighbour lookup efficiency */
            // <<<<<<<<<<!!!>>>>>>>>>>
            ...
            Function<Pedestrian,List<Pedestrian>> getNeighbors = p ->
            {
                return linkedCellsGrid.getObjects(p.getPosition(),
                        attributesSIRG.getInfectionMaxDistance());
            };
            // <<<<<<<<<<!!!>>>>>>>>>>

            /* ### Task 4-6: Decouple infection rate from time-step */
            // <<<<<<<<<<!!!>>>>>>>>>>
            double infectionRatePerDeltaTime = 1-Math.pow(1-attributesSIRG.getInfectionRate(), deltaTime);
            // <<<<<<<<<<!!!>>>>>>>>>>

            /* ### Task 5-2: Decouple recovery rate from time-step */
            // <<<<<<<<<<!!!>>>>>>>>>>
            double recoveryRatePerDeltaTime = 1-Math.pow(1-attributesSIRG.getRecoveryRate(), deltaTime);
            // <<<<<<<<<<!!!>>>>>>>>>>

            for (Pedestrian p : c.getElements()) {
                // loop over neighbors and set infected if we are close
                for (Pedestrian p_neighbor : getNeighbors.apply(p)) {
                    ...
                    if (dist < attributesSIRG.getInfectionMaxDistance() &&
                            this.random.nextDouble() < infectionRatePerDeltaTime) {
            ...
            /* ### Task 5-2: Recovery per time-step */
            // <<<<<<<<<<!!!>>>>>>>>>>
            for (Pedestrian p : c.getElements()) {
                if (getGroup(p).getID() == SIRType.ID_INFECTED.ordinal() &&
                        this.random.nextDouble() < recoveryRatePerDeltaTime) {
                    elementRemoved(p);
        ...
        LastSimulationTime = simTimeInSec;
    }
}